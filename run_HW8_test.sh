...
JAR_LOG="jar_creation.log"
LOG_FILE="combined_output.txt"
GENERAL_LOG="log.txt"
...
# Create the JAR file
echo "Creating crawler.jar..."
jar cvf crawler.jar -C out . >> "$JAR_LOG" 2>&1

# Run the KVS Coordinator
echo "Starting KVS Coordinator on port 8000..." | tee -a "$LOG_FILE"
java -cp out cis5550.kvs.Coordinator 8000 >> "$LOG_FILE" 2>&1 &
sleep 1
PIDS+=($!)

# Run the KVS Worker
echo "Starting KVS Worker on port 8001..." | tee -a "$LOG_FILE"
rm -rf worker1; java -cp out cis5550.kvs.Worker 8001 worker1 localhost:8000 >> "$LOG_FILE" 2>&1 &
sleep 1
PIDS+=($!)

# Run the Flame Coordinator
echo "Starting Flame Coordinator on port 9000..." | tee -a "$LOG_FILE"
java -cp out cis5550.flame.Coordinator 9000 localhost:8000 >> "$LOG_FILE" 2>&1 &
sleep 1
PIDS+=($!)

# Run the Flame Worker
echo "Starting Flame Worker on port 9001..." | tee -a "$LOG_FILE"
java -cp out cis5550.flame.Worker 9001 localhost:9000 >> "$LOG_FILE" 2>&1 &
sleep 1
PIDS+=($!)
